...
# Recording helpers
# -----------------------
def record_audio_from_device(device_index: int, seconds: float, fs: int = 16000) -> np.ndarray:
    audio = sd.rec(
        int(seconds * fs),
        samplerate=fs,
        channels=1,
...
if mode == "Record (one-shot)":
    ...
    if st.button("Record now"):
        fs = 16000
        st.info("Recording...")
        x = record_audio_from_device(device_index, float(dur_s), fs=fs)
...
if mode == "Listen (live-ish)":
    ...
    if st.session_state.get("live_mode", False):
        ...
        x = record_audio_from_device(device_index, float(chunk_s), fs=fs)
        x_trim = trim_silence(x, fs, rel_thresh=float(rel_thr), abs_thresh=float(abs_thr))
        ...
        if x_trim.size > 0:
            ...
            if text_from_audio and text_from_audio != st.session_state.get("last_live_text", ""):
                ...
                # Compute and store so the Results section renders it immediately
                live_result = compute_results(text_from_audio, chosen_override_lang)
                st.session_state["current_result"] = live_result

                # Optional: also push to history automatically
                if live_result.get("ok"):
                    push_history(live_result)

        # Schedule the next loop, but DO NOT rerun yet (let Results render first)
        do_rerun = True
...
if text and text.strip():
    # If live mode already computed this exact text, reuse it
